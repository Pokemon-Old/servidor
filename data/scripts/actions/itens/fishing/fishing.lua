fishingTable = {
	...
	isFishing = 1000001,     -- storage do player / checa se est� pescando
...
function ActionFish.onUse(player, item, fromPosition, target, toPosition)
	...
	if player:isOnRide() or player:isOnFly() or player:isOnSurf() or player:isOnBike() or player:isOnEvent()then
		return player:sendCancelMessage("Voc� n�o pode pescar enquanto est� em condi��o especial.")
	end

	if isInArray(fishingTable.waters, getTileInfo(getThingPos(player)).itemid) then
		return player:sendCancelMessage("Voc� n�o pode pescar se estiver dentro da �gua.")
	end
	...
	if getTileInfo(getThingPos(player:getSummons() and player:getSummons()[1] or player)).protection or getTileInfo(getThingPos(player)).protection then
		return player:sendCancelMessage("Voc� n�o pode pescar se voc� ou seu pokemon estiver em uma zona de prote��o.")
	end
	...
	if isFishFar(player:getPosition(), toPosition) then
		return player:sendTextMessage(MESSAGE_EVENT_ADVANCE, 'Voc� n�o tem for�as suficientes para arremessar at� l�!')
	end

    local isca = player:getIscaPlayer()
	if isca ~= -1 and not player:removeItem(isca, 1) then
		return player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Sem iscas. compre uma isca no vendedor mais pr�ximo para continuar com sua pesca!")
	end
...
function ActionIsca.onUse(player, item, fromPosition, target, toPosition)
    ...
    if player:getSkillLevel(6) < isca.fishLevel then
       return player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Voc� precisa de fishing n�vel: " .. isca.fishLevel .. " para usar essa isca.")
    end

    if player:getLevel() < isca.level then
       return player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Voc� precisa de level " .. isca.level .. " para usar essa isca.")
    end
...
function Player:fishPoke(poke, qnt, toPos)
	...
	if chancePescarTesouro >= math.random(100) then
		...
		self:sendTextMessage(MESSAGE_EVENT_ADVANCE, ("Voc� pescou um tesouro de categoria: %s, meus parabens!"):format(tesouro.rarity))
	...
	if chanceQuebrarLinha >= math.random(100) then
		return true and self:sendTextMessage(MESSAGE_EVENT_ADVANCE, "o pok�mon quebrou a linha e fugiu!!!!!")
	end

	self:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Voc� pescou um " .. poke .. "!")

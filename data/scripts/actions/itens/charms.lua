...
function action.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if player:getStorageValue(charmStorage) - os.time() > 0 then
        ...
        player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, string.format("Voc� acaba de adicionar tempo a: %s, novo tempo: %s", charm.name, convertTime(remainingTime + charm.time)))
        item:remove(1)
        return true
    end
    ...
    doPlayerSendTextMessage(player, MESSAGE_STATUS_CONSOLE_BLUE, string.format("Voc� acaba de ativar um %s, com dura��o de: %s", charm.name, convertTime(charm.time)))
    item:remove(1)
    return true
end

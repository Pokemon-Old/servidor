...
function action_held.onUse(player, item, fromPosition, target, toPosition)
	...
	if hasSummons(player) then
		return player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Guarde seu pok�mon!")
	end

	if not held then
		return error(("[Held System - ACTION]: Item n�o configurado: %d"):format(item.itemid))
	end
    ...
    if not newInfos then
        return error(("[Held System - ACTION]: Held n�o configurado! Ident: %d - Attribute: %s"):format(held.ident, held.attribute))
    end
    ...
    if (verificar == 0 and newTier ~= 1) or (verificar ~= 0 and newTier ~= 1 and oldType ~= newType) then
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Voc� deve utilizar o tier 1 primeiro!")
        return true
    elseif verificar ~= 0 and oldType == newType then
        if oldTier > newTier then
            player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Voc� n�o pode usar um held j� existente no pok�mon!")
            return true
        end

        if oldTier+1 ~= newTier then
            player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Voc� deve seguir a ordem num�rica dos helds. O pr�ximo held deve ser: " .. oldTier+1)
            return true
        end
    end

...
function action.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if not pokemon or target ~= pokemon then
		player:sendCancelMessage("Voc� s� pode evoluir o seu pok�mon.")
		return true
	end
	...
	for _, evolution in ipairs(evolutionList) do
		...
		if canEvolve and stoneFinded then
			...
			if playerLevel < minLevel then
				player:sendCancelMessage("Voc� precisa estar level " .. minLevel .. " para evoluir seu pok�mon.")
				return true
			end
			...
			if pokeChance < randomChance then
				player:sendCancelMessage("A evolu��o do seu pok�mon falhou.")
				return true
			end
			isEvolutionSuccessfull = true
			break
		elseif stoneFinded and not canEvolve then
			player:sendCancelMessage("Voc� n�o possui todas as stones necess�rias para evoluir seu pok�mon.")
			return true
		end
	end

	if not stoneFinded then
		player:sendCancelMessage("Seu pok�mon n�o evolui com essa stone.")
		return true
	end

	if isEvolutionSuccessfull then
		local newPokemon = pokeName
		player:say("Wow, meu " .. pokemon:getName() .. " est� evoluindo!!!", TALKTYPE_ORANGE_1)
